...
const AllMaids = ({ searchTerm }) => {
  ...
  const nationalityFilter = queryParams.get("nationality");
  ...
  const fetchMaids = async (endpoint, key) => {
    ...
    try {
      ...
      if (nationalityFilter) {
        params.countries = nationalityFilter;
      }
  ...
  useEffect(() => {
    const fetchAllMaidData = async () => {
      const [
        ...
        fetchMaids(
          `api/v1/maids/withMonthlyHired/${staffId}`,
          "myMonthlyHired"
        ),
    ...
    fetchAllMaidData();
  }, [isFormVisible, searchTerm, staffId, verifyToken, nationalityFilter]);
  ...
  const renderMaidProfiles = () => {
    ...
    return (
      <div className="w-full border rounded-2xl border-solid p-6 2xl:grid grid-cols-2 gap-2">
        {loading ? (
          Array.from({ length: itemsToLoad[key] }, (_, index) => (
