...
const AccountsCompo = () =>{
    ...
    const [costumerAccountDetails, setCostumerAccountDetails] = useState([])
    const [myCostumerAccountDetails, setMyCostumerAccountDetails] = useState([])
    ...
    useEffect(() => {
        const fetchMyCostumersAccounts = async () => {
          try {
            const response = await axiosInstense.get(
            ...
            setMyCostumerAccountDetails(response.data);
            if(!userRoles.includes(roles.fullAccessOnAccounts)){
              setActiveTab("Costumers")
            }
          } catch (error) {
            console.error("Error fetching maids:", error);
          }
        ...
        fetchMyCostumersAccounts();
      }, []);

    return(
        <>
        <div className="md:ml-[20rem] relative min-h-screen max-h-full md:px-8 px-4">
                {userRoles.includes(roles.fullAccessOnAccounts) && <div>
                ...
                <div>
                  {activeTab === "totalPayment" && userRoles.includes(roles.fullAccessOnAccounts) &&  <div className="relative">
                      ...
                      {accountDetails && costumerAccountDetails ? <div className="w-full rounded-xl bg-[#F2F2F2] overflow-auto gap-4 grid grid-cols-1 sm:flex items-center justify-start border border-solid p-6">
                                <HomeCard cardTxt="Balance Amount" total={staffAccountDetails.totalBalance} count={staffAccountDetails.totalBalance}/>
                                <HomeCard cardTxt="Received Amount" total={staffAccountDetails.totalReceived} count={staffAccountDetails.totalReceived}/>
                                <HomeCard cardTxt="Sent Amount" total={staffAccountDetails.totalSent} count={staffAccountDetails.totalSent}/>
                                <HomeCard cardTxt="Remaining Amount" total={accountDetails.remainingAmount} count={accountDetails.remainingAmount}/>
                ...
                {activeTab === "Costumers" && <div className="costumersAccounts">
                  {userRoles.includes(roles.fullAccessOnAccounts) ? <div className="flex flex-col gap-2 md:gap-4">
                        {costumerAccountDetails
                        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
                        .map(details => (
                            <CostumerAccountDetails key={details._id} accountDetails={details} />
                        ))}
                    </div> : <div className="flex flex-col gap-2 md:gap-4">
                        {myCostumerAccountDetails
                        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
                        .map(details => (
                            <CostumerAccountDetails key={details._id} accountDetails={details} />
                        ))}
