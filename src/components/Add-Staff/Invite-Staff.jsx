...
const InviteStaffForm = ({ onCloseForm }) =>{
    ...
    const [formData, setFormData] = useState({
      ...
      const handleFormSubmit = async (e) => {
        ...
        try {
          const response = await axiosInstense.post('api/v1/staff/invite', formData, {
            headers: {
          ...
          if(response.status === 200){
            setInvitationLink(response.data.inviteLink)
          }
        } catch (error) {
        ...
        <div className="flex items-center justify-between mb-8">
            <div className="text-2xl font-semibold">Invite Staff Member</div>
                  ...
                  <div className="p-4 mb-4 w-full md:w-[26rem] text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                        ...
                        <form onSubmit={handleFormSubmit}>
                            ...
                            <div className="roles">
                                ...
                                <div className="mb-4">
                                    <div style={{borderBottom:".5px solid #C3D0D4"}} class="flex items-center justify-between gap-2 mb-4">
                                        <label htmlFor="canAccessOnVisa" class="mb-2 text-sm md:text-base font-medium text-[#434146]">Have Access On Visa System?</label>
                                        <input  onChange={handleCheckboxChange} name="roles[]" id="canAccessOnVisa" type="checkbox" value="256" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-2xl focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                    </div>
                                </div>
                                {invitationLink && <div className="mb-4">
                                    <label className="form-label block text-xl">Link</label>
                                    <input readOnly value={invitationLink} type="text" className="w-full overflow-auto bg-[#E3E3E3] md:w-[26rem] h-[4rem] outline-none border-none rounded-lg px-2 py-2" />
                                </div>
                                }
                            ...
                            {invitationLink ? <div>
                                <div onClick={handleCopyLink} className="mb-4">
                                    <button type="button" className="w-full flex items-center justify-center text-sm font-semibold bg-[#107243] text-white md:w-[26rem] h-[4rem] outline-none border-none rounded-lg px-2 py-2">
                                        {copied ? "Copied!" : "Copy"}
                                    </button>
                                </div>
                            </div> : <div>
                                <div class="mb-4">
                                    <button type="submit" className="w-full flex items-center justify-center text-sm font-semibold bg-[#107243] text-white md:w-[26rem] h-[4rem] outline-none border-none rounded-lg px-2 py-2">
                                        ...
                                        {!spinningLoader && "Get Invite Link"}
                                    </button>
                                </div>
                            </div>}
