const StaffAccountTransferPaymentHistory = ({ transferHistory, formatDate }) => {
  return (
    <>
      <div
        key={history._id}
        className="historyRow p-3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3"
      >
      <div className="Entry">
        <div className="text-xs">Date</div>
        <div className="text-sm font-semibold">
          {formatDate(transferHistory.date)}
        </div>
      </div>
        <div className="Entry">
          <div className="text-xs">Payment Method</div>
          <div className="text-sm font-semibold">
            {transferHistory.paymentMethod}
          </div>
        </div>
        <div className="Entry">
          <div className="text-xs">Payment Type</div>
          <div className="text-sm font-semibold">
            {transferHistory.type}
          </div>
        </div>

        {transferHistory.type === "Sent" ? (
          <div className="Entry">
            <div className="text-xs">Transfer Amount</div>
            <div className="text-sm font-semibold">
        ...
        {transferHistory.type === "Sent" ? (
          <div className="Entry">
            <div className="text-xs">Transfer To</div>
            <div className="text-sm font-semibold">
              {transferHistory.transferTo}
            </div>
          </div>
        ) : (
          <div className="Entry">
            <div className="text-xs">Received From</div>
            <div className="text-sm font-semibold">
              {transferHistory.receivedFrom}
            </div>
          </div>
        )}

        
        {transferHistory.proof && (
          <div className="Entry justify-self-end w-fit cursor-pointer bg-[#E3E3E3] p-3 rounded-2xl">
            <a
              href={`${import.meta.env.VITE_API_URL}${
                transferHistory.proof
              }`}
              download={`${import.meta.env.VITE_API_URL}${
                transferHistory.proof
              }`}
            >
              <svg
...
export default StaffAccountTransferPaymentHistory;
